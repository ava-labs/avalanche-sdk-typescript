...
describe.skipIf(SKIP_INTEGRATION)("warp + L1 flow against tmpnet", () => {
  ...
  // ───────────────────────────────────────────────────────────────────────
  // Steps 17–20: P-Chain coverage for prepare*Txn methods not exercised by
  // the warp/L1 flow itself. These commit lightweight P-Chain txs against
  // the same tmpnet network. We verify each tx commits and (where cheap)
  // that the resulting on-chain state matches.
  //
  // Skipped from coverage today:
  //   - Cross-chain Import (requires waiting for the export's atomic memory
  //     to surface in destination chain state — adds ~20s with no extra
  //     signal). Export is exercised below; Import is structurally identical.
  // ───────────────────────────────────────────────────────────────────────

  test("17. Export AVAX P → C via prepareExportTxn", async () => {
    const { walletClient, account } = requireState("walletClient", "account");

    // Build a minimal P→C export: 0.01 AVAX (10,000,000 nAVAX) to the
    // owner's own C-Chain bech32 address. The destination is the C-Chain
    // alias; the addresses array holds the recipients on the destination.
    const cChainBech32 = account.getXPAddress("C", "local");
    const txnRequest = await walletClient.pChain.prepareExportTxn({
      destinationChain: "C",
      exportedOutputs: [
        {
          addresses: [cChainBech32],
          amount: 10_000_000n,
        },
      ],
    });
    ...
    await waitForCommitted(walletClient, txHash);
    expect(txHash.length).toBeGreaterThan(0);
    console.log(`[step 17] P→C ExportTx committed: ${txHash}`);
  }, TX_TIMEOUT_MS);

  test("18. Stake on primary network via prepareAddPermissionlessValidatorTxn", async () => {
    const { walletClient, account } = requireState("walletClient", "account");

    // Generate a brand-new BLS keypair for a never-going-to-actually-run
    // validator. We're testing that the TX commits cleanly, not the staking
    // lifecycle — the staked AVAX is locked for the duration but the
    // validator entry just sits in the pending set until end-of-test
    // teardown. Local genesis MinStakeDuration is 24h so we set end well
    // beyond that.
    ...
    // Fresh fake nodeID — the tx still commits because P-Chain doesn't
    // verify the node exists; it just records the staking entry.
    ...
    await waitForCommitted(walletClient, txHash);
    expect(txHash.length).toBeGreaterThan(0);
    console.log(`[step 18] AddPermissionlessValidatorTx committed: ${txHash} (nodeID=${nodeIdB58})`);
  }, TX_TIMEOUT_MS);

  test("19. Delegate to an existing primary network validator", async () => {
    ...
    // Pick the first primary network validator (the bootstrap staker we
    // loaded from STAKING_KEYS_DIR's signer1.key) and delegate to it.
    ...
    const txnRequest = await walletClient.pChain.prepareAddPermissionlessDelegatorTxn({
      // Local genesis MinDelegatorStake = 25 AVAX (in nAvax).
    ...
    expect(txHash.length).toBeGreaterThan(0);
    console.log(
      `[step 19] AddPermissionlessDelegatorTx committed: ${txHash} (delegated to ${target!.nodeId})`,
    );
  }, TX_TIMEOUT_MS);

  test("20. Subnet validator legacy flow (AddSubnetValidator + RemoveSubnetValidator)", async () => {
    ...
    // The legacy permissioned-subnet validator path (pre-ACP77). We can't
    // exercise it against the L1 we built in step 5 because conversion is
    // one-way — instead create a fresh subnet specifically for this test
    // and never convert it.
    const createSubnet = await walletClient.pChain.prepareCreateSubnetTxn({
      subnetOwners: { addresses: [ownerPAddr], threshold: 1 },
    });
    const { txHash: subnet2Tx } = await walletClient.sendXPTransaction({
      tx: createSubnet.tx,
      chainAlias: "P",
    });
    await waitForCommitted(walletClient, subnet2Tx);
    const legacySubnetId = subnet2Tx;
    console.log(`[step 20] created legacy (non-L1) subnet: ${legacySubnetId}`);

    // Add a primary network node as a subnet validator. nodeID must be an
    // actual primary network validator — pick one of the bootstrap nodes.
    const target = primaryNodes[1];
    expect(target).toBeDefined();
    const startNow = BigInt(Math.floor(Date.now() / 1000));
    // Subnet validator MinStakeDuration on local is 24h (same as primary).
    const endTime = startNow + 25n * 3600n;
    ...
    console.log(`[step 20] AddSubnetValidatorTx committed: ${addTx} (${target!.nodeId} → subnet)`);
    ...
    console.log(`[step 20] RemoveSubnetValidatorTx committed: ${removeTx}`);
  }, BOOT_TIMEOUT_MS);
});

...
* [liveCheck](#livecheck) - Get the liveliness of the service (reads only)
...
<!-- UsageSnippet language="typescript" operationID="live-check" method="get" path="/v1/live-check" -->
```typescript
import { Avalanche } from "@avalanche-sdk/chainkit";
...
async function run() {
  const result = await avalanche.data.liveCheck();
...
```typescript
import { AvalancheCore } from "@avalanche-sdk/chainkit/core.js";
import { dataLiveCheck } from "@avalanche-sdk/chainkit/funcs/dataLiveCheck.js";
...
async function run() {
  const res = await dataLiveCheck(avalanche);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("dataLiveCheck failed:", res.error);
  }
}

...
* [getAvaxSupply](#getavaxsupply) - Get AVAX supply information
...
async function run() {
  await avalanche.avaxSupply.getAvaxSupply();
...
import { avaxSupplyGetAVAXSupply } from "@avalanche-sdk/chainkit/funcs/avaxSupplyGetAVAXSupply.js";
...
async function run() {
  const res = await avaxSupplyGetAVAXSupply(avalanche);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("avaxSupplyGetAVAXSupply failed:", res.error);
  }
}

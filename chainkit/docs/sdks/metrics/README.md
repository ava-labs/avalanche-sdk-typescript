...
* [liveCheck](#livecheck) - Get the liveliness of the service
...
<!-- UsageSnippet language="typescript" operationID="metrics-live-check" method="get" path="/v2/live-check" -->
```typescript
import { Avalanche } from "@avalanche-sdk/chainkit";
...
async function run() {
  const result = await avalanche.metrics.liveCheck();
...
```typescript
import { AvalancheCore } from "@avalanche-sdk/chainkit/core.js";
import { metricsLiveCheck } from "@avalanche-sdk/chainkit/funcs/metricsLiveCheck.js";
...
async function run() {
  const res = await metricsLiveCheck(avalanche);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("metricsLiveCheck failed:", res.error);
  }
}

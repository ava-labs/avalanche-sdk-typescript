 ...
 */
export function notificationsUnsubscribe(
  client: AvalancheCore,
  request: components.UnsubscribeRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.NotificationsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.UnsubscribeRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/notifications/unsubscribe")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.NotificationsResponse$inboundSchema),

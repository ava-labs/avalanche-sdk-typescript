...
export const tool$dataPrimaryNetworkTransactionsListLatestPrimaryNetworkTransactions:
  ToolDefinition<typeof args> = {
    name:
      "data-primary-network-transactions-list-latest-primary-network-transactions",
...
Given that each transaction may return a large number of UTXO objects, bounded only by the maximum transaction size, the query may return less transactions than the provided page size. The result will contain less results than the page size if the number of utxos contained in the resulting transactions reach a performance threshold.`,
    ...
    tool: async (client, args, ctx) => {
      const [result, apiCall] =
        await dataPrimaryNetworkTransactionsListLatestPrimaryNetworkTransactions(
          client,
          args.request,

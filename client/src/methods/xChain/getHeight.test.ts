...
import type { GetHeightReturnType } from "./types/getHeight.js";

const mockRequest = vi.fn(async ({ method, params }) => {
  if (method === "avm.getHeight") {
    return {
      height: 100,
    } as GetHeightReturnType;
  }
...
describe("getHeight", () => {
  ...
  test("returns height", async () => {
    const result: GetHeightReturnType = await getHeight(client);

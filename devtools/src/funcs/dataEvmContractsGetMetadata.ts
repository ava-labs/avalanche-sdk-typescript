 ...
 */
export function dataEvmContractsGetMetadata(
  client: AvalancheCore,
  request: operations.GetContractMetadataRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetContractMetadataResponse,
...
> {
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetContractMetadataServerList[0], {
  ...
  const path = pathToFunc("/v1/chains/{chainId}/addresses/{address}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetContractMetadataResponse$inboundSchema),

 ...
 */
export function metricsNetworksGetStakingMetrics(
...
> {
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetStakingMetricsServerList[0], {
  ...
  const path = pathToFunc("/v2/networks/{network}/metrics/{metric}")(
    pathParams,
  );
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetStakingMetricsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      metricsNetworksGetStakingMetrics(
        client,
        {
          ...request,

...
import { GetBlockchainByIdServerList } from "../models/operations/getblockchainbyid.js";
 ...
 */
export function dataPrimaryNetworkGetBlockchainById(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetBlockchainByIdRequest$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetBlockchainByIdServerList[0], {
  ...
  const path = pathToFunc("/v1/networks/{network}/blockchains/{blockchainId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Blockchain$inboundSchema),

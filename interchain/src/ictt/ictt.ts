...
export class ICTT {
  ...
  constructor(
    ...
    this.telemetry = new Telemetry(enableTelemetry);
  }
  ...
  async deployERC20Token(params: DeployERC20TokenParams): Promise<{
    ...
    this.telemetry.captureEvent("ictt_deployERC20Token", {
  ...
  async approveToken(params: ApproveTokenParams): Promise<{
    ...
    this.telemetry.captureEvent("ictt_approveToken", {
      sourceChain: sourceChain.id,
    });
    return approveToken({
  ...
  async deployTokenHomeContract(
    params: DeployTokenHomeContractParams
  ): Promise<{
    txHash: `0x${string}`;
    contractAddress: `0x${string}`;
  }> {
    ...
    this.telemetry.captureEvent("ictt_deployTokenHomeContract", {
      sourceChain: sourceChain.id,
    });
    ...
    const { tokenDecimals } = await getERC20TokenInfo(
      sourceChain,
    ...
    const txHash = await params.walletClient.deployContract({
      ...
      bytecode:
        params.tokenHomeCustomByteCode ??
        (tokenHomeABI.bytecode as `0x${string}`),
      args: [
    ...
    if (receipt.status === "success" && receipt.contractAddress) {
      ...
      throw new Error("Failed to deploy token home contract.", {
        cause: receipt,
      });
    }
  }
  ...
  async deployTokenRemoteContract(
    params: DeployTokenRemoteContractParams
  ): Promise<{
    txHash: `0x${string}`;
    contractAddress: `0x${string}`;
  }> {
    ...
    this.telemetry.captureEvent("ictt_deployTokenRemoteContract", {
      sourceChain: sourceChain.id,
    ...
    const { tokenName, tokenSymbol, tokenDecimals, minTeleporterVersion } =
      await getInfoFromTokenHomeContract(sourceChain, params.tokenHomeContract);

    const txHash = await params.walletClient.deployContract({
      abi: params.tokenRemoteCustomABI ?? tokenRemoteABI.abi,
      bytecode:
        params.tokenRemoteCustomByteCode ??
        (tokenRemoteABI.bytecode as `0x${string}`),
      args: [
        {
          teleporterRegistryAddress:
            destinationChain.interchainContracts.teleporterRegistry,
    ...
    if (receipt.status === "success" && receipt.contractAddress) {
      ...
      throw new Error("Failed to deploy token remote contract.", {
        cause: receipt,
      });
    }
  }
  ...
  async registerRemoteWithHome(params: RegisterRemoteWithHomeParams): Promise<{
    ...
    this.telemetry.captureEvent("ictt_registerRemoteWithHome", {
      sourceChain: sourceChain.id,
      destinationChain: destinationChain.id,
    });
    ...
    if (receipt.status === "success") {
      ...
      throw new Error(
        "Failed to register token remote contract with token home contract.",
        { cause: receipt }
      );
    }
  }
  ...
  async sendToken(params: SendTokenParams): Promise<{
    ...
    this.telemetry.captureEvent("ictt_sendToken", {
      sourceChain: sourceChain.id,
      destinationChain: destinationChain.id,
    });
    ...
    const { tokenContractAddress, tokenDecimals } =
      await getInfoFromTokenHomeContract(sourceChain, params.tokenHomeContract);
    const amountInWei =
      BigInt(params.amountInBaseUnit) * BigInt(10 ** tokenDecimals);

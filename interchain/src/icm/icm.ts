...
export class ICM {
  ...
  constructor(
    ...
    this.requiredGasLimit =
      defaultParams?.requiredGasLimit || this.requiredGasLimit;
    this.allowedRelayerAddresses =
      defaultParams?.allowedRelayerAddresses || this.allowedRelayerAddresses;
  ...
  async sendMsg(params: SendMessageArgs) {
    ...
    if (!sourceChain || !destinationChain) {
      throw new Error(
        "Source or destination chain not set or not provided in the params"
      );
    }
    this.telemetry.captureEvent("icm_sendMsg", {
      sourceChain: sourceChain.id,
      destinationChain: destinationChain.id,
      feeInfo: params.feeInfo,
      requiredGasLimit: params.requiredGasLimit,
    });
    ...
    const messageInput: TeleporterMessageInput = {
      ...
      allowedRelayerAddresses:
        params.allowedRelayerAddresses || this.allowedRelayerAddresses,

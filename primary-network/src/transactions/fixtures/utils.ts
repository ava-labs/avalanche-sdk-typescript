    ...
    expectedOutputs.forEach((expected, index) => {
        ...
        if (actual) {
            if (utils.isTransferOut(actual)) {
                actualOwners = actual?.outputOwners?.addrs?.map(owner => owner.toString('fuji'))
                expect(new Set(actualOwners), `output owners did not match for index ${index}`).toEqual(new Set(expectedOwners))
            } else if (utils.isStakeableLockOut(actual)) {
                actualOwners = actual?.getOutputOwners()?.addrs?.map(owner => owner.toString('fuji'))
                expect(new Set(actualOwners), `stakeable lock output owners did not match for index ${index}`).toEqual(new Set(expectedOwners))
            } else {
